...
$sth = $dbh->query(
    "SELECT students.name, students.mail, entries.training_level, entries.text, students.student_id FROM entries, students 
     WHERE entries.sport_id = '6' AND entries.student_id = students.student_id");
$personen = $sth->fetchAll();
?>

<body>
<div class="überschrift">
    <h1>Sonstige</h1> <a id="entry" href="new_entry.php">Eintrag erstellen</a>
</div>
<article>
    <div class="body">
        <ul>
            <?php  foreach( $personen as $person ) : ?>
                <li>
                    ...
                    if($name == $person->name)
                    {
                        echo "<a id='button' href='delete_entry.php?student_id=".($person->student_id)."&sport_id=6'>Eintrag löschen</a>";
                    }
                    ?>
                </li>
            <?php endforeach; ?>
